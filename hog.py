...
def is_swap(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    if player_score >= 100:
        my_score = abs(((player_score // 10) % 10) - (player_score % 10))
    else:
        my_score = abs((player_score // 10) - (player_score % 10))
    if opponent_score >= 100:
        your_score = abs(((opponent_score // 10) % 10) - (opponent_score % 10))
    else:
        your_score = abs((opponent_score // 10) - (opponent_score % 10))
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    # BEGIN PROBLEM 5
    num_rolls, player_score, opponent_score = 0, 0, 0
    while score0 < goal and score1 < goal:
        if player == 0:
            player_score = score0
            opponent_score = score1
            num_rolls = strategy0(player_score, opponent_score)
        elif player == 1:
            player_score = score1
            opponent_score = score0
            num_rolls = strategy1(player_score, opponent_score)
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def say(score0, score1):
        ...
        if highest_gain < score_diff:
            highest_gain = score_diff
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 11
    updated_player_score = score + free_bacon(opponent_score)  # new score if rolled 0
    if is_swap(updated_player_score, opponent_score):
        if not(updated_player_score > opponent_score):
            return 0
    elif free_bacon(opponent_score) >= margin:
        if not(is_swap(updated_player_score, opponent_score)):
            return 0
    return num_rolls
    # END PROBLEM 11
